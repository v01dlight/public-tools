...
def parse_arguments():
    ...
    parser.add_argument('-e', '--email', type=str, default="user@outlook.com", help="Email address to use for the Outlook connection")
    ...
    parser.add_argument('-q', '--quiet', action=argparse.BooleanOptionalAction, help="Quiet mode: suppresses banner and printing extracted data to terminal")
...
def print_columns_to_terminal(headers, data, columns_to_print):    
    print(", ".join([row[i] for row in data for i in columns_to_print]))
            ...
            if message.Class == 43: # Ensure it's a mail item
                ...
                if message.FlagStatus != 3 and message.FlagStatus != 1 and message.FlagStatus != 2 and re.search(target_subject, subject):
                    ...
                    if table:
                        ...
                        if not args.quiet:
                            print_columns_to_terminal(headers, data, columns_to_print)
                            print()

                        # Increment the extraction counter
                        num_extracted += 1
                    else:
                        print(f"[!] No table found in email")
